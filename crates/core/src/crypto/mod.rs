...
//! `secp256k1_ecrecover` through [`zeth_r0vm_precompile`]. All other `Crypto`
//! methods fall through to the upstream revm default implementation.
//!
//! On host builds [`install_r0vm_crypto`] is a no-op — `zeth-r0vm-precompile`
...
impl reth_evm::revm::precompile::Crypto for R0vmCrypto {
    ...
    fn modexp(
        ...
        exp: &[u8],
        modulus: &[u8],
    ) -> Result<Vec<u8>, reth_evm::revm::precompile::PrecompileError> {
    ...
    fn bn254_g1_add(
        ...
        p1: &[u8],
        p2: &[u8],
    ) -> Result<[u8; 64], reth_evm::revm::precompile::PrecompileError> {
        zeth_r0vm_precompile::bn254_g1_add(p1, p2)
            .ok_or(reth_evm::revm::precompile::PrecompileError::Bn254AffineGFailedToCreate)
    }
    ...
    fn secp256k1_ecrecover(
        ...
        recid: u8,
        msg: &[u8; 32],
    ) -> Result<[u8; 32], reth_evm::revm::precompile::PrecompileError> {
        zeth_r0vm_precompile::secp256k1_ecrecover(sig, recid, msg)
            .map(|addr| addr.into_word().0)
            .ok_or(reth_evm::revm::precompile::PrecompileError::Secp256k1RecoverFailed)
    }
}
